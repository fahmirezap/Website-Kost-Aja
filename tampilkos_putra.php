...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="tampilkosputra.css">
    <style>
      form {
        margin-left: 150px;
      }
  ...
  <div class="jumbotron">
    <div class="container">
        <h1>Kost Putra</h1><br>
        <p>Berisi Daftar Kost Laki-laki</p>
      </div>
  </div>
  ...
  <?php 
  include "koneksi.php";
  $query = "SELECT k.*,p.* FROM kost k, pemilik p WHERE k.id_pemilik = p.id_pemilik AND jenis_kost='putra' ";
                   ...
                   <div class="col-sm-4 col-lg-4 col-md-4">
                         <?php
                              if(isset($_GET['cari']))
                              {
                                $cari = $_GET['cari'];
                                include 'koneksi.php';
                                $result=mysqli_query($konek, "SELECT k.*,p.* FROM kost k, pemilik p WHERE k.id_pemilik = p.id_pemilik AND jenis_kost='putra' AND kota LIKE '%".$cari."%'");
                              }
                              else
                              {
                                include 'koneksi.php';
                                $result=mysqli_query($konek, "SELECT k.*,p.* FROM kost k, pemilik p WHERE k.id_pemilik = p.id_pemilik AND jenis_kost='putra'");
                              }
